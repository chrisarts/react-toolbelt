...
const atom = <T>(initialValue: T | AtomGetter<T>): Atom<T> => {
  ...
  return {
    ...
    cleanup: () => {
      for (const sb of subscribers) {
        subscribers.delete(sb);
      }
      for (const sb of subscribed) {
        subscribed.delete(sb);
      }
    },
  };
};
